...
/// accept `arena = "owned"` (the default) or `arena = "shared"`. CMSIS-NN
/// lowering is opt-in with `cmsis_nn = true`.
...
pub fn model(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let expanded = args::ModelArgs::parse(attr).and_then(|args| {
        ...
        backend::expand(args.backend, args.arena, args.cmsis_nn, input_struct, model)
    });
    match expanded {
